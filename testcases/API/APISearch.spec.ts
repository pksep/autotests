...
import { SearchAPI } from '../../pages/API/APISearch';
...
export const runSearchAPI = () => {
  ...
  test('Search API - Security & Authentication Tests', async ({ request, page }) => {
    ...
    const searchAPI = new SearchAPI(page);
    ...
    await test.step('Test 1: Search without authentication', async () => {
      ...
      const unauthenticatedResponse = await searchAPI.searchGlobal(request, searchData);
    ...
    await test.step('Test 2: Search with SQL injection in query', async () => {
      ...
      const sqlInjectionResponse = await searchAPI.searchGlobal(request, sqlInjectionData);
    ...
    await test.step('Test 3: Search with XSS payload', async () => {
      ...
      const xssResponse = await searchAPI.searchGlobal(request, xssData);
  ...
  test('Search API - Data Validation & Edge Cases', async ({ request, page }) => {
    ...
    await test.step('Test 4: Search with invalid data types', async () => {
      ...
      const invalidSearchResponse = await searchAPI.searchGlobal(request, invalidData);

      // API PROBLEM: If this returns 200, data validation is missing
      expect.soft(invalidSearchResponse.status).toBe(400);
      expect.soft(invalidSearchResponse.status).not.toBe(200);
      expect.soft(invalidSearchResponse.status).not.toBe(201);
      expect.soft(invalidSearchResponse.status).not.toBe(401);
      expect.soft(invalidSearchResponse.status).not.toBe(500);
      expect.soft(invalidSearchResponse.status).not.toBe(502);
      expect.soft(invalidSearchResponse.status).not.toBe(503);
      expect.soft(invalidSearchResponse.status).not.toBe(504);
      // Catch-all: Any other status code indicates API inconsistency
      expect.soft([400, 422, 401]).toContain(invalidSearchResponse.status);
      expect.soft(invalidSearchResponse.data).toBeDefined();
    ...
    await test.step('Test 5: Search with empty query', async () => {
      ...
      const emptySearchResponse = await searchAPI.searchGlobal(request, emptyData);

      // API PROBLEM: If this returns 200, empty query validation is missing
      expect.soft(emptySearchResponse.status).toBe(400);
      expect.soft(emptySearchResponse.status).not.toBe(200);
      expect.soft(emptySearchResponse.status).not.toBe(201);
      expect.soft(emptySearchResponse.status).not.toBe(401);
      expect.soft(emptySearchResponse.status).not.toBe(500);
      expect.soft(emptySearchResponse.status).not.toBe(502);
      expect.soft(emptySearchResponse.status).not.toBe(503);
      expect.soft(emptySearchResponse.status).not.toBe(504);
      // Catch-all: Any other status code indicates API inconsistency
      expect.soft([400, 422, 401]).toContain(emptySearchResponse.status);
      expect.soft(emptySearchResponse.data).toBeDefined();
  ...
  test('Search API - Performance & Concurrency Tests', async ({ request, page }) => {
    ...
    await test.step('Test 6: Response time performance for search', async () => {
      ...
      const performanceSearchResponse = await searchAPI.searchGlobal(request, searchData);
      const endTime = Date.now();
      const responseTime = endTime - startTime;

      // API PROBLEM: If response time is too slow, there's a performance issue
      expect.soft(performanceSearchResponse.status).toBe(200);
    ...
    await test.step('Test 7: Test concurrent search attempts', async () => {
      ...
      const promises = Array(5)
        .fill(null)
        .map(() => searchAPI.searchGlobal(request, searchData));
