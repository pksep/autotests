...
import { PartsAPI } from '../../pages/API/APIParts';
...
export const runPartsAPI = () => {
  ...
  test('Parts API - Security & Authentication Tests', async ({ request, page }) => {
    ...
    await test.step('Test 1: Create part without authentication', async () => {
      ...
      logger.log('✅ Unauthenticated part creation correctly rejected with 401');
    });
    ...
    await test.step('Test 2: Create part with SQL injection in name', async () => {
      ...
      const sqlInjectionResponse = await partsAPI.createPart(request, sqlInjectionData, token);
    ...
    await test.step('Test 3: Create part with XSS payload', async () => {
      ...
      const xssResponse = await partsAPI.createPart(request, xssData, token);
  ...
  test('Parts API - Data Validation & Edge Cases', async ({ request, page }) => {
    ...
    await test.step('Test 4: Create part with invalid data types', async () => {
      ...
      const invalidCreateResponse = await partsAPI.createPart(request, invalidData, authToken);
    ...
    await test.step('Test 5: Create part with empty required fields', async () => {
      ...
      const emptyCreateResponse = await partsAPI.createPart(request, emptyData, authToken);
  ...
  test('Parts API - Performance & Concurrency Tests', async ({ request, page }) => {
    ...
    await test.step('Test 6: Response time performance for create part', async () => {
      logger.log('Testing create part response time performance...');
      ...
      const performanceCreateResponse = await partsAPI.createPart(request, partData, authToken);
      ...
      logger.log(`✅ Create part response time: ${responseTime}ms (acceptable)`);
    });

    await test.step('Test 7: Test concurrent part operations', async () => {
      logger.log('Testing concurrent part operations...');
      ...
      const promises = Array(5)
        .fill(null)
        .map(() => partsAPI.createPart(request, partData, authToken));
      ...
      logger.log('✅ Concurrent part operations handled successfully');
    });
  });
};
