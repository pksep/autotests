...
export const runDetailsAPI = () => {
    ...
    await test.step('Add Detail File', async () => {
      ...
      if (nonExistentResponse.status === 404) {
        ...
        if (nonExistentResponse.status === 201) {
          ...
          logger.log(
            `   8. Body: {"id": 1234567890123456, "name": "Test Detail", "designation": "TEST.001", "description": "Test description", "quantity": 0, "min_remaining": 0, "remainder_after_coming": 0, "deficit": 0, "shipments_kolvo": 0, "production_ordered": 0, "deficit_by_sclad": 0, "attention": false, "discontinued": false, "responsibleId": 4}`,
          );
      ...
      if (response.status === 201) {
        ...
        if (response.data) {
          ...
          if (response.data.success === true || response.data.message?.includes('success') || response.data.id || response.data.fileId) {
  ...
  test('Security - SQL injection attempts', async ({ request }) => {
    ...
    if (response.status === 401 || response.status === 400) {
      ...
      expect.soft([401, 400]).toContain(response.status);
    }
  });
  ...
  test('Validation - Duplicate designation prevention', async ({ request }) => {
    ...
    await test.step('Test Duplicate Designation', async () => {
      ...
      const hasError = response.data && response.data.status === 'error' && response.data.message && response.data.message.includes('уже существует');
  ...
  test('Summary - Results Overview', async ({ request }) => {
