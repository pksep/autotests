...
import { TemplatesAPI } from '../../pages/API/APITemplates';
import { AuthAPI } from '../../pages/API/APIAuth';
import { ENV } from '../../config';
import { API_CONST } from '../../lib/Constants/APIConstants';
import logger from '../../lib/utils/logger';

export const runTemplatesAPI = () => {
  logger.info(`Starting Templates API defensive tests - looking for API problems`);

  test('Templates API - Security & Authentication Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const templatesAPI = new TemplatesAPI(page);
    const authAPI = new AuthAPI(page);

    await test.step('Test 1: Create template without authentication', async () => {
      logger.log('Testing unauthenticated template creation...');

      const templateData = {
        name: API_CONST.API_TEST_TEMPLATE_NAME,
        category: API_CONST.API_TEST_TEMPLATE_CATEGORY,
      };

      const unauthenticatedResponse = await templatesAPI.createTemplate(request, templateData, 'invalid_user');
      ...
      logger.log('✅ Unauthenticated template creation correctly rejected with 401');
    });

    await test.step('Test 2: Create template with SQL injection in name', async () => {
      ...
      const sqlInjectionResponse = await templatesAPI.createTemplate(request, sqlInjectionData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 3: Create template with XSS payload', async () => {
      ...
      const xssResponse = await templatesAPI.createTemplate(request, xssData, API_CONST.API_TEST_USER_ID);
  ...
  test('Templates API - Data Validation & Edge Cases', async ({ request, page }) => {
    test.setTimeout(60000);
    const templatesAPI = new TemplatesAPI(page);
    const authAPI = new AuthAPI(page);
    ...
    await test.step('Test 4: Create template with invalid data types', async () => {
      logger.log('Testing data type validation...');

      const invalidData = {
        name: API_CONST.API_TEST_EDGE_CASES.INVALID_NUMBER,
        category: ['invalid'], // Should be string, not array
      };

      const invalidCreateResponse = await templatesAPI.createTemplate(request, invalidData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 5: Create template with empty required fields', async () => {
      ...
      const emptyCreateResponse = await templatesAPI.createTemplate(request, emptyData, API_CONST.API_TEST_USER_ID);
  ...
  test('Templates API - Performance & Concurrency Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const templatesAPI = new TemplatesAPI(page);
    const authAPI = new AuthAPI(page);
    let authToken: string;
    ...
    await test.step('Test 6: Response time performance for create template', async () => {
      logger.log('Testing create template response time performance...');
      ...
      const performanceCreateResponse = await templatesAPI.createTemplate(request, templateData, API_CONST.API_TEST_USER_ID);
      ...
      logger.log(`✅ Create template response time: ${responseTime}ms (acceptable)`);
    });

    await test.step('Test 7: Test concurrent template creation attempts', async () => {
      logger.log('Testing concurrent template creation attempts...');

      const templateData = {
        name: API_CONST.API_TEST_TEMPLATE_NAME_UPDATED,
        category: API_CONST.API_TEST_TEMPLATE_CATEGORY_UPDATED,
      };

      const promises = Array(5)
        .fill(null)
        .map(() => templatesAPI.createTemplate(request, templateData, API_CONST.API_TEST_USER_ID));
      ...
      logger.log('✅ Concurrent template creation attempts handled successfully');
    });
  });
};
