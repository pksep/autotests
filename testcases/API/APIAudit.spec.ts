...
import { AuditAPI } from '../../pages/API/APIAudit';
...
export const runAuditAPI = () => {
  ...
  test('Audit API - Security & Authentication Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const auditAPI = new AuditAPI(page);
    ...
    await test.step('Test 2: Create audit entry with SQL injection', async () => {
      ...
      const sqlInjectionData = {
        ...
        resource: API_CONST.API_TEST_AUDIT_RESOURCE,
        result: API_CONST.API_TEST_AUDIT_RESULT,
      };

      const sqlInjectionResponse = await auditAPI.createAuditEntry(request, sqlInjectionData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 3: Create audit entry with XSS payload', async () => {
      ...
      const xssResponse = await auditAPI.createAuditEntry(request, xssData, API_CONST.API_TEST_USER_ID);
  ...
  test('Audit API - Data Validation & Edge Cases', async ({ request, page }) => {
    test.setTimeout(60000);
    const auditAPI = new AuditAPI(page);
    const authAPI = new AuthAPI(page);
    ...
    await test.step('Test 4: Create audit entry with invalid data types', async () => {
      ...
      const invalidData = {
        ...
        resource: ['invalid'], // Should be string, not array
        result: API_CONST.API_TEST_AUDIT_RESULT,
      };

      const invalidCreateResponse = await auditAPI.createAuditEntry(request, invalidData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 5: Create audit entry with empty required fields', async () => {
      ...
      const emptyData = {
        ...
        resource: API_CONST.API_TEST_EDGE_CASES.EMPTY_STRING,
        result: API_CONST.API_TEST_EDGE_CASES.EMPTY_STRING,
      };

      const emptyCreateResponse = await auditAPI.createAuditEntry(request, emptyData, API_CONST.API_TEST_USER_ID);
  ...
  test('Audit API - Performance & Concurrency Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const auditAPI = new AuditAPI(page);
    const authAPI = new AuthAPI(page);
    let authToken: string;
    ...
    await test.step('Test 7: Test concurrent audit operations', async () => {
      ...
      const auditData = {
        ...
        resource: API_CONST.API_TEST_AUDIT_RESOURCE_UPDATED,
        result: API_CONST.API_TEST_AUDIT_RESULT_UPDATED,
      };

      const promises = Array(5)
        .fill(null)
        .map(() => auditAPI.createAuditEntry(request, auditData, API_CONST.API_TEST_USER_ID));
