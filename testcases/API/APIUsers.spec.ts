...
export const runUsersAPI = () => {
  ...
  test('Users API - Security & Authentication Tests', async ({ request }) => {
    ...
    await test.step('Test 4: Get users without authentication', async () => {
      ...
      if (unauthenticatedResponse.status === 200) {
        ...
        if (typeof unauthenticatedResponse.data === 'string' && String(unauthenticatedResponse.data).includes('<html>')) {
    ...
    if (securityIssuesFound > 0) {
      ...
      //expect.soft(securityIssuesFound, `Security audit found ${securityIssuesFound} vulnerabilities in Users API`).toBe(0);
      //ERP-2055
    } else {
  ...
  test('Users API - Data Validation & Edge Cases', async ({ request }) => {
    ...
    await test.step('Test 5: Create user with invalid data types', async () => {
      ...
      expect.soft(invalidCreateResponse.status).toBe(401);
    ...
    await test.step('Test 6: Create user with empty required fields', async () => {
      ...
      expect.soft(emptyResponse.status).toBe(401);
    ...
    await test.step('Test 7: Create user with extremely long initials', async () => {
      ...
      expect.soft(longInitialsResponse.status).toBe(401);
      expect.soft(longInitialsResponse.status).not.toBe(201);
      expect.soft(longInitialsResponse.status).not.toBe(200);
      expect.soft(longInitialsResponse.data).toBeDefined();
    ...
    await test.step('Test 9: Update user with invalid data', async () => {
      ...
      expect.soft([400, 401]).toContain(invalidUpdateResponse.status);
    ...
    await test.step('Test 10: Update non-existent user', async () => {
      ...
      // For defensive testing, we document the issue but don't fail the test
      expect.soft(nonExistentUpdateResponse.status).not.toBe(200); // Should never succeed
    });
    ...
    await test.step('Test 11: Update user with valid data', async () => {
      ...
      // expect.soft(updateResponse.data.initials).toBe(API_CONST.API_TEST_USER_INITIALS_UPDATED);
      // expect.soft(updateResponse.data.tabel).toBe(API_CONST.API_TEST_USER_TABEL_UPDATED);
      //ERP-2155
  ...
  test('Users API - CRUD Operations & Data Integrity', async ({ request }) => {
    ...
    await test.step('Test 12: Test pagination with invalid data', async () => {
      ...
      try {
        ...
        expect.soft([400, 401]).toContain(invalidPaginationResponse.status);
        expect.soft(invalidPaginationResponse.status).not.toBe(200);
        expect.soft(invalidPaginationResponse.data).toBeDefined();
    ...
    await test.step('Test 13: Test archive users with invalid data', async () => {
      ...
      try {
        ...
        if (invalidArchiveResponse) {
          ...
          expect.soft([400, 401]).toContain(invalidArchiveResponse.status);
          expect.soft(invalidArchiveResponse.status).not.toBe(200);
          expect.soft(invalidArchiveResponse.data).toBeDefined();
    ...
    await test.step('Test 14: Test valid operations', async () => {
      ...
      expect.soft(usersListResponse.status).toBe(200);
      expect.soft(usersListResponse.status).not.toBe(400);
      expect.soft(usersListResponse.status).not.toBe(401);
      expect.soft(usersListResponse.data).toBeDefined();
      expect.soft(Array.isArray(usersListResponse.data)).toBe(true);
  ...
  test('Users API - Role Management & User Operations', async ({ request }) => {
    ...
    await test.step('2: Create a test user for safe operations', async () => {
      ...
      try {
        ...
        if (roleUsersResponse) {
          ...
          if (roleUsersResponse.status === 200) {
            expect.soft(Array.isArray(roleUsersResponse.data)).toBe(true);
      ...
      try {
        ...
        if (userResponse) {
          ...
          if (userResponse.status === 200) {
            expect.soft(userResponse.data).toBeDefined();
