...
export const runAuthAPI = () => {
    ...
    test("Test 1:Auth API - Security & Authentication Tests", async ({ request }, testInfo) => {
        ...
        if (totalFindings > 0) {
            ...
            const shortSummary = findings
                .map((f) => `[${f.severity}] ${f.step}: ${f.issue}`)
                .join("; ");
            expect.soft(
                totalFindings,
                `SECURITY AUDIT FAILED (${totalFindings} issue(s)). Open the "security-audit-failure-report.txt" attachment for full details. Summary: ${shortSummary}`
            ).toBe(0);
        } else {
