...
export const runAuthAPI = () => {
  ...
  test('1:Auth API - Security & Authentication Tests', async ({ request }, testInfo) => {
    ...
    let tokenValidationBypassed = false;
    /** Collected findings for clear failure report: { step, issue, severity, endpoint, expected, actual } */
    const findings: { step: string; issue: string; severity: string; endpoint: string; expected: string; actual: string }[] = [];

    await test.step('Test 1: Login with invalid credentials', async () => {
      logger.log('Testing invalid credentials...');

      const invalidLoginResponse = await authAPI.login(request, 'invalid_user', 'invalid_password', 'invalid_tabel');
      ...
      if (actualStatus === expectedStatus) {
        ...
        const issue = `Invalid credentials returned ${actualStatus}, expected ${expectedStatus}`;
        logger.log(`❌ SECURITY ISSUE: ${issue}`);
        ...
        findings.push({
          ...
          issue,
          severity: 'MEDIUM',
          endpoint: ENV.API_BASE_URL + 'api/auth/login (POST)',
          expected: '401',
      ...
      if (!invalidLoginResponse.data) {
        ...
        findings.push({
          step: 'Login with invalid credentials',
          issue: 'No response data returned',
          severity: 'LOW',
          endpoint: ENV.API_BASE_URL + 'api/auth/login (POST)',
          expected: 'Response body present',
    ...
    await test.step('Test 2: Login with empty credentials', async () => {
      ...
      const emptyLoginResponse = await authAPI.login(request, API_CONST.API_TEST_EDGE_CASES.EMPTY_STRING, API_CONST.API_TEST_EDGE_CASES.EMPTY_STRING, API_CONST.API_TEST_EDGE_CASES.EMPTY_STRING);
      ...
      if (actualStatus === expectedStatus) {
        ...
        findings.push({
          ...
          issue: `Empty credentials returned ${actualStatus}, expected ${expectedStatus}`,
          severity: 'MEDIUM',
          endpoint: ENV.API_BASE_URL + 'api/auth/login (POST)',
          expected: '401',
          actual: String(actualStatus),
    ...
    await test.step('Test 3: Login with SQL injection in username', async () => {
      ...
      const sqlInjectionResponse = await authAPI.login(request, API_CONST.API_TEST_EDGE_CASES.SQL_INJECTION_USERNAME, 'password', '12345');
      ...
      if (actualStatus === expectedStatus) {
        ...
        securityIssuesFound++;
        findings.push({
          step: 'Login with SQL injection in username',
          issue: `SQL injection returned ${actualStatus}, expected ${expectedStatus}`,
          severity: 'HIGH',
          endpoint: ENV.API_BASE_URL + 'api/auth/login (POST)',
          expected: '401',
          actual: String(actualStatus),
    ...
    await test.step('Test 4: Login with XSS payload in username', async () => {
      ...
      const xssResponse = await authAPI.login(request, API_CONST.API_TEST_EDGE_CASES.XSS_PAYLOAD, 'password', '12345');
      ...
      if (actualStatus === expectedStatus) {
        ...
        securityIssuesFound++;
        findings.push({
          step: 'Login with XSS payload in username',
          issue: `XSS returned ${actualStatus}, expected ${expectedStatus}`,
          severity: 'HIGH',
          endpoint: ENV.API_BASE_URL + 'api/auth/login (POST)',
          expected: '401',
          actual: String(actualStatus),
    ...
    const userEndpoint = ENV.API_BASE_URL + 'api/auth/user (GET)';
    ...
    await test.step('Test 5: Get user with invalid token', async () => {
      ...
      const invalidTokenResponse = await authAPI.getUserByToken(request, API_CONST.API_TEST_EDGE_CASES.INVALID_TOKEN);
      ...
      if (actualStatus === 200) {
        ...
        findings.push({
          ...
          issue: 'Invalid token accepted with 200 OK — token validation bypassed',
          severity: 'CRITICAL',
        ...
        findings.push({
          step: 'Get user with invalid token',
          issue: `Invalid token returned ${actualStatus}, expected ${expectedStatus}`,
          severity: 'MEDIUM',
          endpoint: userEndpoint,
          expected: '401',
          actual: String(actualStatus),
    ...
    await test.step('Test 6: Get user with malformed token', async () => {
      ...
      const malformedTokenResponse = await authAPI.getUserByToken(request, API_CONST.API_TEST_EDGE_CASES.MALFORMED_TOKEN);
      ...
      if (actualStatus === 200) {
        ...
        tokenValidationBypassed = true;
        findings.push({
          step: 'Get user with malformed token',
          issue: 'Malformed token accepted with 200 OK — token validation bypassed',
          severity: 'CRITICAL',
          endpoint: userEndpoint,
          expected: '401',
          actual: '200',
        ...
        findings.push({
          step: 'Get user with malformed token',
          issue: `Malformed token returned ${actualStatus}, expected ${expectedStatus}`,
          severity: 'MEDIUM',
          endpoint: userEndpoint,
          expected: '401',
          actual: String(actualStatus),
    ...
    await test.step('Test 7: Get user with empty token', async () => {
      ...
      const emptyTokenResponse = await authAPI.getUserByToken(request, API_CONST.API_TEST_EDGE_CASES.EMPTY_STRING);
      ...
      if (actualStatus === 200) {
        ...
        tokenValidationBypassed = true;
        findings.push({
          step: 'Get user with empty token',
          issue: 'Empty token accepted with 200 OK — token validation bypassed',
          severity: 'CRITICAL',
          endpoint: userEndpoint,
          expected: '401',
          actual: '200',
        ...
        findings.push({
          step: 'Get user with empty token',
          issue: `Empty token returned ${actualStatus}, expected ${expectedStatus}`,
          severity: 'MEDIUM',
          endpoint: userEndpoint,
          expected: '401',
          actual: String(actualStatus),
    ...
    if (tokenValidationBypassed) {
      securityIssuesFound = Math.max(securityIssuesFound, 1);
    }

    logger.log(`\n🔍 SECURITY AUDIT SUMMARY:`);
    logger.log(`   Total security issues found: ${findings.length}`);

    const totalFindings = findings.length;
    if (totalFindings > 0) {
      logger.log(`\n🚨 SECURITY AUDIT RESULT: FAILED`);
      logger.log(`   ${totalFindings} security issue(s) detected!`);
      logger.log(`\n📋 RECOMMENDED ACTIONS:`);
      ...
      // Build a clear report for the tester (file attachment + assertion message)
      const reportLines: string[] = [
        '═══════════════════════════════════════════════════════════════',
        '  AUTH API SECURITY AUDIT — FAILURE REPORT',
        '═══════════════════════════════════════════════════════════════',
        '',
        'WHAT FAILED:',
        ...findings.map((f, i) => {
          return [`  ${i + 1}. [${f.severity}] ${f.step}`, `     Issue: ${f.issue}`, `     Endpoint: ${f.endpoint}`, `     Expected: ${f.expected}  |  Actual: ${f.actual}`, ''].join('\n');
        }),
        'WHERE TO FIX:',
        '  • Login issues → ' + ENV.API_BASE_URL + 'api/auth/login (POST)',
        '  • Token validation → ' + ENV.API_BASE_URL + 'api/auth/user (GET)',
        '',
        'RECOMMENDED ACTIONS:',
        '  1. Ensure GET api/auth/user returns 401 for invalid/malformed/empty Authorization header.',
        '  2. Ensure POST api/auth/login returns 401 for invalid/empty/SQL/XSS payloads.',
        '  3. Re-run this test after fixes.',
        '═══════════════════════════════════════════════════════════════',
      ];
      const reportText = reportLines.join('\n');
      await testInfo.attach('security-audit-failure-report.txt', {
        body: reportText,
        contentType: 'text/plain',
      });

      const shortSummary = findings.map(f => `[${f.severity}] ${f.step}: ${f.issue}`).join('; ');
      expect.soft(totalFindings, `SECURITY AUDIT FAILED (${totalFindings} issue(s)). Open the "security-audit-failure-report.txt" attachment for full details. Summary: ${shortSummary}`).toBe(0);
    } else {
  ...
  test('2:Auth API - Login Process Test', async ({ request }) => {
    test.setTimeout(600000);
    const authAPI = new AuthAPI(null as any);
    let capturedToken: string; // Store token for use across steps

    await test.step('1: Test login security - valid credentials should work', async () => {
      ...
      if (loginResponse.status === 201) {
        ...
        expect.soft(loginResponse.status, 'CRITICAL: Valid credentials should not cause server error').toBe(201);
        ...
        expect.soft(loginResponse.status, 'CRITICAL: Valid credentials should not cause Bad Gateway error').toBe(201);
      } else {
      ...
      if (!loginResponse.data || typeof loginResponse.data !== 'string' || loginResponse.data.length === 0) {
        ...
        expect.soft(loginResponse.data, 'SECURITY ISSUE: Token should be a non-empty string').toBeTruthy();
      }
    ...
    await test.step('2: Test token validation security', async () => {
      logger.log('🔍 DEFENSIVE TEST: Testing token validation security');

      // Use the stored token from Step 1
      expect.soft(capturedToken).toBeDefined();
      ...
      if (tokenResponse.status === 200) {
        ...
        expect.soft(tokenResponse.status, 'CRITICAL: Valid token should not be rejected').toBe(200);
        ...
        logger.log('🚨 SEVERITY: CRITICAL - Authentication system failure');
        expect.soft(tokenResponse.status, 'CRITICAL: Token validation should not cause server error').toBe(200);
      } else {
    ...
    await test.step('3: Test credential format security - invalid formats should be rejected', async () => {
      ...
      if (securityIssuesFound > 0) {
        ...
        expect.soft(securityIssuesFound, `SECURITY ISSUE: ${securityIssuesFound} invalid credential formats were accepted`).toBe(0);
      } else {
        logger.log(`✅ Credential format security test passed - all invalid formats rejected`);
      }
    });

    await test.step('4: Test token structure security', async () => {
      ...
      if (capturedToken.length < 100) {
        ...
        expect.soft(capturedToken.length, 'SECURITY ISSUE: Token should be at least 100 characters').toBeGreaterThan(100);
      ...
      if (capturedToken.includes('password') || capturedToken.includes('admin') || capturedToken.includes('test')) {
        ...
        expect.soft(capturedToken, 'CRITICAL: Token should not contain obvious patterns').not.toMatch(/(password|admin|test)/gi);
      } else {
      ...
      if (foundPatterns.length > 0) {
        ...
        logger.log('🚨 SEVERITY: HIGH - Weak token generation');
        expect.soft(foundPatterns.length, 'SECURITY ISSUE: Token should not contain common patterns').toBe(0);
      } else {
  ...
  test('3:Auth API - Login Functionality Diagnostic', async ({ request }) => {
    test.setTimeout(60000);
    const authAPI = new AuthAPI(null as any);

    await test.step('1: Verify API endpoint reachability', async () => {
      const healthCheck = await request.get(ENV.API_BASE_URL);

      expect.soft(healthCheck.status()).toBeDefined();
      expect.soft(healthCheck.status()).not.toBe(500);

      logger.log(`✅ API server reachable - Status: ${healthCheck.status()}`);
    });

    await test.step('2: Test login endpoint connectivity', async () => {
      ...
      expect.soft(testResponse.status).toBeDefined();
      expect.soft(testResponse.data).toBeDefined();
      ...
      if (testResponse.status === 400 || testResponse.status === 401) {
        ...
        //expect.soft([400, 401], "SECURITY ISSUE: Invalid credentials should return 400/401, not 500").toContain(testResponse.status);
        //ERP-2153
        ...
        logger.log('🚨 SEVERITY: CRITICAL - Complete authentication bypass');
        expect.soft([400, 401], 'CRITICAL: Invalid credentials should not be accepted').toContain(testResponse.status);
      } else {
        ...
        expect.soft([400, 401]).toContain(testResponse.status);
      }

      logger.log(`✅ Login endpoint connectivity test completed - Status: ${testResponse.status}`);
    });

    await test.step('3: Test edge case credential validation', async () => {
      ...
      if (securityIssuesFound > 0) {
        ...
        expect.soft(securityIssuesFound, `SECURITY ISSUE: ${securityIssuesFound} edge cases were accepted`).toBe(0);
      } else {
  ...
  test('4:Auth API - Data Validation & Edge Cases', async ({ request }) => {
    test.setTimeout(60000);
    const authAPI = new AuthAPI(null as any);

    await test.step('1: Test extremely long username validation', async () => {
      const longUsernameResponse = await authAPI.login(request, API_CONST.API_TEST_EDGE_CASES.VERY_LONG_STRING, 'password', '12345');

      // API consistently returns 401 for invalid inputs (correct security behavior)
      expect.soft(longUsernameResponse.status).toBe(401);
      expect.soft(longUsernameResponse.data).toBeDefined();

      logger.log(`✅ Long username validation successful - Status: ${longUsernameResponse.status}`);
    });

    await test.step('2: Test special characters validation', async () => {
      const specialCharsResponse = await authAPI.login(request, API_CONST.API_TEST_EDGE_CASES.SPECIAL_CHARACTERS, API_CONST.API_TEST_EDGE_CASES.SPECIAL_CHARACTERS, API_CONST.API_TEST_EDGE_CASES.SPECIAL_CHARACTERS);

      expect.soft(specialCharsResponse.status).toBe(401);
      expect.soft(specialCharsResponse.data).toBeDefined();

      logger.log(`✅ Special characters validation successful - Status: ${specialCharsResponse.status}`);
    });

    await test.step('3: Test Unicode characters validation', async () => {
      const unicodeResponse = await authAPI.login(request, API_CONST.API_TEST_EDGE_CASES.UNICODE_CHARACTERS, 'password', '12345');

      expect.soft(unicodeResponse.status).toBe(401);
      expect.soft(unicodeResponse.data).toBeDefined();

      logger.log(`✅ Unicode characters validation successful - Status: ${unicodeResponse.status}`);
    });

    await test.step('4: Test invalid data types validation', async () => {
      const numberUsernameResponse = await authAPI.login(request, API_CONST.API_TEST_EDGE_CASES.INVALID_NUMBER as any, 'password', '12345');

      expect.soft(numberUsernameResponse.status).toBe(401);
      expect.soft(numberUsernameResponse.data).toBeDefined();

      logger.log(`✅ Invalid data types validation successful - Status: ${numberUsernameResponse.status}`);
    });
  });

  test('5:Auth API - Token Validation & Security', async ({ request }) => {
    test.setTimeout(60000);
    const authAPI = new AuthAPI(null as any);
    let validToken: string; // Store token for use across steps

    await test.step('1: Obtain valid authentication token', async () => {
      const loginResponse = await authAPI.login(request, API_CONST.API_TEST_USERNAME, API_CONST.API_TEST_PASSWORD, API_CONST.API_TEST_TABEL);
    ...
    await test.step('2: Test token validation endpoint', async () => {
      const validTokenResponse = await authAPI.getUserByToken(request, validToken);

      expect.soft(validTokenResponse.status).toBe(200);
      expect.soft(validTokenResponse.data).toBeDefined();
      ...
      if (validTokenResponse.status === 200) {
        ...
        expect.soft(validTokenResponse.data).toHaveProperty('user');
      } else {
    ...
    await test.step('3: Test token expiration handling', async () => {
    ...
    await test.step('4: Test concurrent token usage', async () => {
      ...
      try {
        ...
        const promises = Array(5)
          .fill(null)
          .map(() => authAPI.getUserByToken(request, validToken));
