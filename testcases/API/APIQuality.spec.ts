...
import { QualityAPI } from '../../pages/API/APIQuality';
import { AuthAPI } from '../../pages/API/APIAuth';
import { ENV } from '../../config';
import { API_CONST } from '../../lib/Constants/APIConstants';
import logger from '../../lib/utils/logger';

export const runQualityAPI = () => {
  logger.info(`Starting Quality API defensive tests - looking for API problems`);

  test('Quality API - Security & Authentication Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const qualityAPI = new QualityAPI(page);
    const authAPI = new AuthAPI(page);

    await test.step('Test 1: Create quality check without authentication', async () => {
      logger.log('Testing unauthenticated quality check creation...');

      const qualityData = {
        name: API_CONST.API_TEST_QUALITY_CHECK_NAME,
        status: API_CONST.API_TEST_QUALITY_CHECK_STATUS,
        score: API_CONST.API_TEST_QUALITY_CHECK_SCORE,
      };

      const unauthenticatedResponse = await qualityAPI.createQualityCheck(request, qualityData, 'invalid_user');
      ...
      logger.log('✅ Unauthenticated quality check creation correctly rejected with 401');
    });

    await test.step('Test 2: Create quality check with SQL injection in name', async () => {
      ...
      const sqlInjectionResponse = await qualityAPI.createQualityCheck(request, sqlInjectionData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 3: Create quality check with XSS payload', async () => {
      ...
      const xssResponse = await qualityAPI.createQualityCheck(request, xssData, API_CONST.API_TEST_USER_ID);
  ...
  test('Quality API - Data Validation & Edge Cases', async ({ request, page }) => {
    test.setTimeout(60000);
    const qualityAPI = new QualityAPI(page);
    const authAPI = new AuthAPI(page);
    ...
    await test.step('Test 4: Create quality check with invalid data types', async () => {
      ...
      const invalidCreateResponse = await qualityAPI.createQualityCheck(request, invalidData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 5: Create quality check with empty required fields', async () => {
      ...
      const emptyData = {
        ...
        score: API_CONST.API_TEST_EDGE_CASES.EMPTY_STRING,
      };

      const emptyCreateResponse = await qualityAPI.createQualityCheck(request, emptyData, API_CONST.API_TEST_USER_ID);
  ...
  test('Quality API - Performance & Concurrency Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const qualityAPI = new QualityAPI(page);
    const authAPI = new AuthAPI(page);
    let authToken: string;

    await test.step('1: Authenticate user for performance tests', async () => {
      logger.log('Authenticating user...');
    ...
    await test.step('Test 6: Response time performance for create quality check', async () => {
      logger.log('Testing create quality check response time performance...');
      ...
      const performanceCreateResponse = await qualityAPI.createQualityCheck(request, qualityData, API_CONST.API_TEST_USER_ID);
      ...
      logger.log(`✅ Create quality check response time: ${responseTime}ms (acceptable)`);
    });

    await test.step('Test 7: Test concurrent quality operations', async () => {
      logger.log('Testing concurrent quality operations...');

      const qualityData = {
        name: API_CONST.API_TEST_QUALITY_CHECK_NAME_UPDATED,
        status: API_CONST.API_TEST_QUALITY_CHECK_STATUS_UPDATED,
        score: API_CONST.API_TEST_QUALITY_CHECK_SCORE_UPDATED,
      };

      const promises = Array(5)
        .fill(null)
        .map(() => qualityAPI.createQualityCheck(request, qualityData, API_CONST.API_TEST_USER_ID));
      ...
      logger.log('✅ Concurrent quality operations handled successfully');
    });
  });
};
