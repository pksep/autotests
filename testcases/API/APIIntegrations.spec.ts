...
import { IntegrationsAPI } from '../../pages/API/APIIntegrations';
...
export const runIntegrationsAPI = () => {
  ...
  test('Integrations API - Security & Authentication Tests', async ({ request, page }) => {
    ...
    const integrationsAPI = new IntegrationsAPI(page);
    const authAPI = new AuthAPI(page);

    await test.step('Test 1: Create integration without authentication', async () => {
      logger.log('Testing unauthenticated integration creation...');
      ...
      logger.log('✅ Unauthenticated integration creation correctly rejected with 401');
    });
  ...
  test('Integrations API - Performance & Concurrency Tests', async ({ request, page }) => {
    ...
    await test.step('Test 6: Response time performance for create integration', async () => {
      logger.log('Testing create integration response time performance...');
      ...
      logger.log(`✅ Create integration response time: ${responseTime}ms (acceptable)`);
    });

    await test.step('Test 7: Test concurrent integration creation attempts', async () => {
      logger.log('Testing concurrent integration creation attempts...');
      ...
      const promises = Array(5)
        .fill(null)
        .map(() => integrationsAPI.createIntegration(request, integrationData, API_CONST.API_TEST_USER_ID));
      ...
      logger.log('✅ Concurrent integration creation attempts handled successfully');
    });
  });
};
