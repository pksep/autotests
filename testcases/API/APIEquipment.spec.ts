...
export const runEquipmentAPI = () => {
  logger.info(`Starting Equipment API defensive tests - looking for API problems`);

  test('Equipment API - Security & Authentication Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const equipmentAPI = new EquipmentAPI(page);
    const authAPI = new AuthAPI(page);

    await test.step('Test 1: Create equipment without authentication', async () => {
      logger.log('Testing unauthenticated equipment creation...');

      const equipmentData = {
        name: API_CONST.API_TEST_EQUIPMENT_NAME,
        type: API_CONST.API_TEST_EQUIPMENT_TYPE,
        status: API_CONST.API_TEST_EQUIPMENT_STATUS,
      };

      const unauthenticatedResponse = await equipmentAPI.createEquipmentType(request, equipmentData, 'invalid_user');
      ...
      logger.log('✅ Unauthenticated equipment creation correctly rejected with 401');
    });

    await test.step('Test 2: Create equipment with SQL injection in name', async () => {
      ...
      const sqlInjectionResponse = await equipmentAPI.createEquipmentType(request, sqlInjectionData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 3: Create equipment with XSS payload', async () => {
      ...
      const xssResponse = await equipmentAPI.createEquipmentType(request, xssData, API_CONST.API_TEST_USER_ID);
  ...
  test('Equipment API - Data Validation & Edge Cases', async ({ request, page }) => {
    test.setTimeout(60000);
    const equipmentAPI = new EquipmentAPI(page);
    const authAPI = new AuthAPI(page);
    ...
    await test.step('Test 4: Create equipment with invalid data types', async () => {
      ...
      const invalidCreateResponse = await equipmentAPI.createEquipmentType(request, invalidData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 5: Create equipment with empty required fields', async () => {
      ...
      const emptyCreateResponse = await equipmentAPI.createEquipmentType(request, emptyData, API_CONST.API_TEST_USER_ID);
  ...
  test('Equipment API - Performance & Concurrency Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const equipmentAPI = new EquipmentAPI(page);
    const authAPI = new AuthAPI(page);
    let authToken: string;
    ...
    await test.step('Test 6: Response time performance for create equipment', async () => {
      logger.log('Testing create equipment response time performance...');
      ...
      const performanceCreateResponse = await equipmentAPI.createEquipmentType(request, equipmentData, API_CONST.API_TEST_USER_ID);
      ...
      logger.log(`✅ Create equipment response time: ${responseTime}ms (acceptable)`);
    });

    await test.step('Test 7: Test concurrent equipment operations', async () => {
      logger.log('Testing concurrent equipment operations...');

      const equipmentData = {
        name: API_CONST.API_TEST_EQUIPMENT_NAME_UPDATED,
        type: API_CONST.API_TEST_EQUIPMENT_TYPE_UPDATED,
        status: API_CONST.API_TEST_EQUIPMENT_STATUS_UPDATED,
      };

      const promises = Array(5)
        .fill(null)
        .map(() => equipmentAPI.createEquipmentType(request, equipmentData, API_CONST.API_TEST_USER_ID));
      ...
      logger.log('✅ Concurrent equipment operations handled successfully');
    });
  });
};
