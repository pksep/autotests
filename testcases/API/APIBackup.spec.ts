...
import { BackupAPI } from '../../pages/API/APIBackup';
import { AuthAPI } from '../../pages/API/APIAuth';
import { ENV } from '../../config';
import { API_CONST } from '../../lib/Constants/APIConstants';
import logger from '../../lib/utils/logger';

export const runBackupAPI = () => {
  logger.info(`Starting Backup API defensive tests - looking for API problems`);

  test('Backup API - Security & Authentication Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const backupAPI = new BackupAPI(page);
    const authAPI = new AuthAPI(page);

    await test.step('Test 1: Create backup without authentication', async () => {
      logger.log('Testing unauthenticated backup creation...');

      const backupData = {
        name: API_CONST.API_TEST_BACKUP_NAME,
        type: API_CONST.API_TEST_BACKUP_TYPE,
        status: API_CONST.API_TEST_BACKUP_STATUS,
      };

      const unauthenticatedResponse = await backupAPI.createBackup(request, backupData, 'invalid_user');
      ...
      logger.log('✅ Unauthenticated backup creation correctly rejected with 401');
    });

    await test.step('Test 2: Create backup with SQL injection in name', async () => {
      ...
      const sqlInjectionResponse = await backupAPI.createBackup(request, sqlInjectionData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 3: Create backup with XSS payload', async () => {
      ...
      const xssResponse = await backupAPI.createBackup(request, xssData, API_CONST.API_TEST_USER_ID);
  ...
  test('Backup API - Data Validation & Edge Cases', async ({ request, page }) => {
    test.setTimeout(60000);
    const backupAPI = new BackupAPI(page);
    const authAPI = new AuthAPI(page);
    ...
    await test.step('Test 4: Create backup with invalid data types', async () => {
      ...
      const invalidCreateResponse = await backupAPI.createBackup(request, invalidData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 5: Create backup with empty required fields', async () => {
      ...
      const emptyCreateResponse = await backupAPI.createBackup(request, emptyData, API_CONST.API_TEST_USER_ID);
  ...
  test('Backup API - Performance & Concurrency Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const backupAPI = new BackupAPI(page);
    const authAPI = new AuthAPI(page);
    let authToken: string;
    ...
    await test.step('Test 6: Response time performance for create backup', async () => {
      logger.log('Testing create backup response time performance...');
      ...
      const performanceCreateResponse = await backupAPI.createBackup(request, backupData, API_CONST.API_TEST_USER_ID);
      ...
      logger.log(`✅ Create backup response time: ${responseTime}ms (acceptable)`);
    });

    await test.step('Test 7: Test concurrent backup creation attempts', async () => {
      logger.log('Testing concurrent backup creation attempts...');

      const backupData = {
        name: API_CONST.API_TEST_BACKUP_NAME_UPDATED,
        type: API_CONST.API_TEST_BACKUP_TYPE_UPDATED,
        status: API_CONST.API_TEST_BACKUP_STATUS_UPDATED,
      };

      const promises = Array(5)
        .fill(null)
        .map(() => backupAPI.createBackup(request, backupData, API_CONST.API_TEST_USER_ID));
      ...
      logger.log('✅ Concurrent backup creation attempts handled successfully');
    });
  });
};
