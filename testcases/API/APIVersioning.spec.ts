...
import { VersioningAPI } from '../../pages/API/APIVersioning';
import { AuthAPI } from '../../pages/API/APIAuth';
import { ENV } from '../../config';
import { API_CONST } from '../../lib/Constants/APIConstants';
import logger from '../../lib/utils/logger';

export const runVersioningAPI = () => {
  logger.info(`Starting Versioning API defensive tests - looking for API problems`);

  test('Versioning API - Security & Authentication Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const versioningAPI = new VersioningAPI(page);
    const authAPI = new AuthAPI(page);

    await test.step('Test 1: Create version without authentication', async () => {
      logger.log('Testing unauthenticated version creation...');

      const versionData = {
        number: API_CONST.API_TEST_VERSION_NUMBER,
        status: API_CONST.API_TEST_VERSION_STATUS,
      };

      const unauthenticatedResponse = await versioningAPI.createVersion(request, versionData, 'invalid_user');
      ...
      logger.log('✅ Unauthenticated version creation correctly rejected with 401');
    });

    await test.step('Test 2: Create version with SQL injection in number', async () => {
      logger.log('Testing SQL injection protection...');

      const sqlInjectionData = {
        number: API_CONST.API_TEST_EDGE_CASES.SQL_INJECTION_USERNAME,
        status: API_CONST.API_TEST_VERSION_STATUS,
      };

      const sqlInjectionResponse = await versioningAPI.createVersion(request, sqlInjectionData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 3: Create version with XSS payload', async () => {
      logger.log('Testing XSS protection...');

      const xssData = {
        number: API_CONST.API_TEST_EDGE_CASES.XSS_PAYLOAD,
        status: API_CONST.API_TEST_VERSION_STATUS,
      };

      const xssResponse = await versioningAPI.createVersion(request, xssData, API_CONST.API_TEST_USER_ID);
  ...
  test('Versioning API - Data Validation & Edge Cases', async ({ request, page }) => {
    test.setTimeout(60000);
    const versioningAPI = new VersioningAPI(page);
    const authAPI = new AuthAPI(page);
    ...
    await test.step('Test 4: Create version with invalid data types', async () => {
      logger.log('Testing data type validation...');

      const invalidData = {
        number: API_CONST.API_TEST_EDGE_CASES.INVALID_NUMBER,
        status: ['invalid'], // Should be string, not array
      };

      const invalidCreateResponse = await versioningAPI.createVersion(request, invalidData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 5: Create version with empty required fields', async () => {
      ...
      const emptyCreateResponse = await versioningAPI.createVersion(request, emptyData, API_CONST.API_TEST_USER_ID);
  ...
  test('Versioning API - Performance & Concurrency Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const versioningAPI = new VersioningAPI(page);
    const authAPI = new AuthAPI(page);
    let authToken: string;
    ...
    await test.step('Test 6: Response time performance for create version', async () => {
      logger.log('Testing create version response time performance...');
      ...
      const performanceCreateResponse = await versioningAPI.createVersion(request, versionData, API_CONST.API_TEST_USER_ID);
      ...
      logger.log(`✅ Create version response time: ${responseTime}ms (acceptable)`);
    });

    await test.step('Test 7: Test concurrent version creation attempts', async () => {
      logger.log('Testing concurrent version creation attempts...');

      const versionData = {
        number: API_CONST.API_TEST_VERSION_NUMBER_UPDATED,
        status: API_CONST.API_TEST_VERSION_STATUS_UPDATED,
      };

      const promises = Array(5)
        .fill(null)
        .map(() => versioningAPI.createVersion(request, versionData, API_CONST.API_TEST_USER_ID));
      ...
      logger.log('✅ Concurrent version creation attempts handled successfully');
    });
  });
};
