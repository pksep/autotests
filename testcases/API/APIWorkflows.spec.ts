...
import { WorkflowsAPI } from '../../pages/API/APIWorkflows';
import { AuthAPI } from '../../pages/API/APIAuth';
import { ENV } from '../../config';
import { API_CONST } from '../../lib/Constants/APIConstants';
import logger from '../../lib/utils/logger';

export const runWorkflowsAPI = () => {
  logger.info(`Starting Workflows API defensive tests - looking for API problems`);

  test('Workflows API - Security & Authentication Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const workflowsAPI = new WorkflowsAPI(page);
    const authAPI = new AuthAPI(page);

    await test.step('Test 1: Create workflow without authentication', async () => {
      logger.log('Testing unauthenticated workflow creation...');

      const workflowData = {
        name: API_CONST.API_TEST_WORKFLOW_NAME,
        status: API_CONST.API_TEST_WORKFLOW_STATUS,
      };

      const unauthenticatedResponse = await workflowsAPI.createWorkflow(request, workflowData, 'invalid_user');
      ...
      logger.log('✅ Unauthenticated workflow creation correctly rejected with 401');
    });

    await test.step('Test 2: Create workflow with SQL injection in name', async () => {
      ...
      const sqlInjectionResponse = await workflowsAPI.createWorkflow(request, sqlInjectionData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 3: Create workflow with XSS payload', async () => {
      ...
      const xssResponse = await workflowsAPI.createWorkflow(request, xssData, API_CONST.API_TEST_USER_ID);
  ...
  test('Workflows API - Data Validation & Edge Cases', async ({ request, page }) => {
    test.setTimeout(60000);
    const workflowsAPI = new WorkflowsAPI(page);
    const authAPI = new AuthAPI(page);
    ...
    await test.step('Test 4: Create workflow with invalid data types', async () => {
      ...
      const invalidCreateResponse = await workflowsAPI.createWorkflow(request, invalidData, API_CONST.API_TEST_USER_ID);
    ...
    await test.step('Test 5: Create workflow with empty required fields', async () => {
      ...
      const emptyCreateResponse = await workflowsAPI.createWorkflow(request, emptyData, API_CONST.API_TEST_USER_ID);
  ...
  test('Workflows API - Performance & Concurrency Tests', async ({ request, page }) => {
    test.setTimeout(60000);
    const workflowsAPI = new WorkflowsAPI(page);
    const authAPI = new AuthAPI(page);
    let authToken: string;
    ...
    await test.step('Test 6: Response time performance for create workflow', async () => {
      logger.log('Testing create workflow response time performance...');
      ...
      const performanceCreateResponse = await workflowsAPI.createWorkflow(request, workflowData, API_CONST.API_TEST_USER_ID);
      ...
      logger.log(`✅ Create workflow response time: ${responseTime}ms (acceptable)`);
    });

    await test.step('Test 7: Test concurrent workflow creation attempts', async () => {
      logger.log('Testing concurrent workflow creation attempts...');

      const workflowData = {
        name: API_CONST.API_TEST_WORKFLOW_NAME_UPDATED,
        status: API_CONST.API_TEST_WORKFLOW_STATUS_UPDATED,
      };

      const promises = Array(5)
        .fill(null)
        .map(() => workflowsAPI.createWorkflow(request, workflowData, API_CONST.API_TEST_USER_ID));
      ...
      logger.log('✅ Concurrent workflow creation attempts handled successfully');
    });
  });
};
