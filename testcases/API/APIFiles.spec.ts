...
import { FilesAPI } from '../../pages/API/APIFiles';
...
export const runFilesAPI = () => {
  ...
  test('Files API - Data Validation & Edge Cases', async ({ request, page }) => {
    ...
    await test.step('Test 4: Upload file with invalid data types', async () => {
      ...
      expect.soft(invalidUploadResponse.status).toBe(400);
      expect.soft(invalidUploadResponse.status).not.toBe(201);
      expect.soft(invalidUploadResponse.status).not.toBe(200);
      expect.soft(invalidUploadResponse.status).not.toBe(401);
      expect.soft(invalidUploadResponse.status).not.toBe(500);
      expect.soft(invalidUploadResponse.status).not.toBe(502);
      expect.soft(invalidUploadResponse.status).not.toBe(503);
      expect.soft(invalidUploadResponse.status).not.toBe(504);
      // Catch-all: Any other status code indicates API inconsistency
      expect.soft([400, 422, 401]).toContain(invalidUploadResponse.status);
      expect.soft(invalidUploadResponse.data).toBeDefined();
    ...
    await test.step('Test 5: Upload file with empty required fields', async () => {
      ...
      expect.soft(emptyUploadResponse.status).toBe(400);
      expect.soft(emptyUploadResponse.status).not.toBe(201);
      expect.soft(emptyUploadResponse.status).not.toBe(200);
      expect.soft(emptyUploadResponse.status).not.toBe(401);
      expect.soft(emptyUploadResponse.status).not.toBe(500);
      expect.soft(emptyUploadResponse.status).not.toBe(502);
      expect.soft(emptyUploadResponse.status).not.toBe(503);
      expect.soft(emptyUploadResponse.status).not.toBe(504);
      // Catch-all: Any other status code indicates API inconsistency
      expect.soft([400, 422, 401]).toContain(emptyUploadResponse.status);
      expect.soft(emptyUploadResponse.data).toBeDefined();
  ...
  test('Files API - Performance & Concurrency Tests', async ({ request, page }) => {
    ...
    await test.step('Test 6: Response time performance for get files', async () => {
      ...
      const performanceGetResponse = await filesAPI.getAllFiles(request, {});
    ...
    await test.step('Test 7: Test concurrent file operations', async () => {
      ...
      const promises = Array(5)
        .fill(null)
        .map(() => filesAPI.uploadFile(request, fileData, API_CONST.API_TEST_USER_ID));
