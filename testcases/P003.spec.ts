...
import testData from '../testdata/SA18-T2.json'; // Import your test data
import testData2 from '../testdata/SA18-T1.json'; // Import your test data

const WAREHOUSE_PAGE_BUTTON = 'Sclad-stockOrderAssembly';
const RESET_FILTERS = 'AssemblySclad-ShipmentsOrder-ButtonClear';
const SHOW_LEFT_TABLE_BUTTON = 'AssemblySclad-ShipmentsOrder-ButtonHidden';
...
export const runP003 = () => {
  ...
  test('Case 0: Заказ склада на сборку Page - Scan tables within a specific element', async ({ page }) => {
  ...
  test('Case 1 - Verify Сборка Склад (Assembly Warehouse) Page Column Count and Order Check for RIGHT table', async ({ page }) => {
    allure.label('severity', 'normal');
    allure.label('epic', 'Склад');
    allure.label('feature', 'Заказ склада на Сборку');
    allure.label('story', 'Verify Column count and order');
    allure.description('Verify Сборка Склад (Assembly Warehouse) Page Column Count and Order Check for RIGHT table.');
  ...
  test('Case 2 - Verify Сборка Склад (Assembly Warehouse) Page Column Count and Order Check for LEFT table', async ({ page }) => {
    allure.label('severity', 'normal');
    allure.label('epic', 'Склад');
    allure.label('feature', 'Заказ склада на Сборку');
    allure.label('story', 'Verify Column count and order');
    allure.description('Verify Сборка Склад (Assembly Warehouse) Page Column Count and Order Check for LEFT table.');
    const shortagePage = new CreateAssemblyWarehousePage(page);
  ...
  test('Case 3 - Verify Сборка Склад (Assembly Warehouse) Page Column header values Check for RIGHT table', async ({ page }) => {
    allure.label('severity', 'normal');
    allure.label('epic', 'Склад');
    allure.label('feature', 'Заказ склада на Сборку');
  ...
  test('Case 4 - Verify Сборка Склад (Assembly Warehouse) Page Column header values Check for LEFT table', async ({ page }) => {
    allure.label('severity', 'normal');
    allure.label('epic', 'Склад');
    allure.label('feature', 'Заказ склада на Сборку');
    allure.label('story', 'Verify Column header values check');
  ...
  test('Case 5 - Verify Сборка Склад (Assembly Warehouse) Page Row Ordering for RIGHT table', async ({ page }) => {
    test.setTimeout(600000);
    allure.label('severity', 'normal');
    allure.label('epic', 'Склад');
    allure.label('feature', 'Заказ склада на Сборку');
  ...
  test('Case 6 - Verify Сборка Склад (Assembly Warehouse) Page Row Ordering for LEFT table', async ({ page }) => {
    allure.label('severity', 'normal');
    allure.label('epic', 'Склад');
    allure.label('feature', 'Заказ склада на Сборку');
    allure.label('story', 'Verify row sort ordering');
  ...
  test('Case 7 - Verify Сборка Склад (Assembly Warehouse) Page search functionality LEFT table', async ({ page }) => {
    allure.label('severity', 'normal');
    allure.label('epic', 'Склад');
    allure.label('feature', 'Заказ склада на Сборку');
    allure.label('story', 'Verify row sort ordering');
    ...
    await allure.step('Step 5: Check Search Functionality', async () => {
      ...
      await allure.step('5.3: Extract text from searchable columns of the first valid row for testing data', async () => {
        ...
        while (!found) {
          const row = page.locator(`[data-testid="${tableId}"] tbody tr:nth-child(${rowIndex})`);
      ...
      await allure.step('5.7: Measure the performance of the search functionality', async () => {
        ...
        const results = page.locator(`[data-testid="${LEFT_DATA_TABLE_CELL_X}"]`);
  ...
  test('Case 8 - Verify Сборка Склад (Assembly Warehouse) Page search functionality RIGHT table', async ({ page }) => {
    allure.label('severity', 'normal');
    allure.label('epic', 'Склад');
    allure.label('feature', 'Заказ склада на Сборку');
    allure.label('story', 'Verify row sort ordering');
    ...
    await allure.step('Step 4: Check Search Functionality', async () => {
      ...
      await allure.step('5.3: Extract text from searchable columns of the first valid row for testing data', async () => {
        ...
        while (!found && rowIndex <= totalRowCount) {
          const row = page.locator(`${tableSelector}:nth-child(${rowIndex})`);
          const cells = row.locator('td');
          const cellCount = await cells.count();
      ...
      await allure.step('5.7: Measure the performance of the search functionality', async () => {
        ...
        const results = page.locator(`[data-testid="${RIGHT_DATA_TABLE_CELL_X}"]`);
  ...
  test('Case 9 - Verify Сборка Склад (Assembly Warehouse) Page - Compare dates with Order List for RIGHT table', async ({ page }) => {
    test.setTimeout(600000);
    allure.label('severity', 'normal');
    allure.label('epic', 'Склад');
    allure.label('feature', 'Заказ склада на Сборку');
    ...
    await allure.step('Step 4: compare the dates in each Row, with thier Orders list', async () => {
      ...
      if (nameColId !== -1 && urgencyColId !== -1 && plannedShipmentColId !== -1 && ordersColId !== -1) {
        ...
        const correctDatesFound = await shortagePage.checkDatesWithOrderList(
          page,
          RIGHT_DATA_TABLE,
          nameColId,
          urgencyColId,
        ...
        const missingCol = nameColId === -1 ? 'Name' : urgencyColId === -1 ? 'Дата по срочности' : plannedShipmentColId === -1 ? 'Дата план. отгрузки' : 'Orders';
        throw new Error(`Column "${missingCol}" not found`);
      }
    });
  });
