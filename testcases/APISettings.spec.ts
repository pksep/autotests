...
export const runSettingsAPI = () => {
    ...
    test("Settings API - Security & Authentication Tests", async ({ request, page }) => {
        ...
        await test.step("Test 1: Update settings without authentication", async () => {
            ...
            const unauthenticatedResponse = await settingsAPI.updateSettings(request, settingsData, "invalid_user");
            ...
            console.log("✅ Unauthenticated settings update correctly rejected with 401");
        });
        ...
        await test.step("Test 2: Update settings with SQL injection", async () => {
            ...
            const sqlInjectionResponse = await settingsAPI.updateSettings(request, sqlInjectionData, API_CONST.API_TEST_USER_ID);
        ...
        await test.step("Test 3: Update settings with XSS payload", async () => {
            ...
            const xssResponse = await settingsAPI.updateSettings(request, xssData, API_CONST.API_TEST_USER_ID);
    ...
    test("Settings API - Data Validation & Edge Cases", async ({ request, page }) => {
        ...
        await test.step("Test 4: Update settings with invalid data types", async () => {
            ...
            const invalidUpdateResponse = await settingsAPI.updateSettings(request, invalidData, API_CONST.API_TEST_USER_ID);
        ...
        await test.step("Test 5: Update settings with empty required fields", async () => {
            ...
            const emptyUpdateResponse = await settingsAPI.updateSettings(request, emptyData, API_CONST.API_TEST_USER_ID);
    ...
    test("Settings API - Performance & Concurrency Tests", async ({ request, page }) => {
        ...
        await test.step("Test 6: Response time performance for get settings", async () => {
            console.log("Testing get settings response time performance...");

            const startTime = Date.now();
            const performanceGetResponse = await settingsAPI.getSettings(request, API_CONST.API_TEST_USER_ID);
            ...
            console.log(`✅ Get settings response time: ${responseTime}ms (acceptable)`);
        });

        await test.step("Test 7: Test concurrent settings operations", async () => {
            console.log("Testing concurrent settings operations...");
            ...
            const promises = Array(5).fill(null).map(() =>
                settingsAPI.updateSettings(request, settingsData, API_CONST.API_TEST_USER_ID)
            );
            ...
            console.log("✅ Concurrent settings operations handled successfully");
        });
    });
};
