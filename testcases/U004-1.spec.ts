...
// U004 script-specific test data (unique for parallel runs; same pattern as ERP-3015)
const U004_PREFIX = 'U004_';
const {
...
/**
 * U004 suite: script-specific test data and cleanup (parallel-safe).
 * All names use U004_ prefix; no Т15 or shared catalog. Same pattern as ERP-3015.
 *
 * Test Case 00: U001-style cleanup (clear search, refresh, search+archive loop until empty for each table),
 * then create 3 CBEDs, 1 detail, 2 materials (ПД), consumables (РМ) if configured, and the product with full specification.
 */
export const runU004_1 = () => {
  logger.info(`Starting test U004`);

  test('Test Case 00 - Create U004 product and spec (cleanup by prefix then create)', async ({ page }, testInfo) => {
    logger.info('Test Case 00 - U001-style cleanup U004_* then create spec items (including materials) and product');
    test.setTimeout(240000);
    const partsDatabasePage = new CreatePartsDatabasePage(page);
    const materialsPage = new CreateMaterialsDatabasePage(page);

    // Scope search inputs to their table/wrapper so we target the correct table (avoid nth() ambiguity)
    const searchProduct = page.locator(SelectorsPartsDataBase.PRODUCT_TABLE).locator(SelectorsPartsDataBase.SEARCH_PRODUCT_ATTRIBUT);
    const searchCbed = page.locator(SelectorsPartsDataBase.CBED_TABLE_WRAPPER).locator(SelectorsPartsDataBase.SEARCH_CBED_ATTRIBUT);
    const searchDetail = page.locator(SelectorsPartsDataBase.DETAIL_TABLE_DIV).locator(SelectorsPartsDataBase.SEARCH_DETAIL_ATTRIBUT);
    ...
    // Order: Products first (so CBEDs are not "in use"), then CBED, then Details
    await allure.step('Step 02: Process Product table - search and archive all items starting with U004_', async () => {
      await searchProduct.fill(U004_PREFIX);
      await searchProduct.press('Enter');
      await partsDatabasePage.waitForNetworkIdle();
      ...
      while (hasMoreItems && iterationCount < maxIterations) {
        ...
        for (let i = rowCount - 1; i >= 0; i--) {
          ...
          if (cellText?.trim().startsWith(U004_PREFIX)) {
            await row.click();
    ...
    await allure.step('Step 02b: Refresh the page after Products cleanup', async () => {
      await page.reload();
      await partsDatabasePage.waitForNetworkIdle();
      await page.waitForTimeout(TIMEOUTS.MEDIUM);
    });

    await allure.step('Step 03: Process CBED table - search and archive all items starting with U004_', async () => {
      await searchCbed.fill(U004_PREFIX);
      await searchCbed.press('Enter');
      await partsDatabasePage.waitForNetworkIdle();
      await page.waitForTimeout(TIMEOUTS.STANDARD);
      let hasMoreItems = true;
    ...
    await allure.step('Step 04: Process Details table - search and archive all items starting with U004_', async () => {
      await searchDetail.fill(U004_PREFIX);
      await searchDetail.press('Enter');
      await partsDatabasePage.waitForNetworkIdle();
      await page.waitForTimeout(TIMEOUTS.STANDARD);
      let hasMoreItems = true;
    ...
    await allure.step('Step 05: Clean up U004 materials (ПД) by prefix', async () => {
      await materialsPage.cleanupTestMaterialsByPrefix(U004_PREFIX);
    });

    // Create in reverse order: materials first (stay on materials page), then detail, then assemblies, then product (no going back).
    await allure.step('Step 06: Create U004 materials (ПД) in Materials DB', async () => {
      for (const mat of U004_STANDARD_PARTS) {
        await materialsPage.createMaterial(mat.name, testInfo);
      }
    });
    await allure.step('Step 07: Create U004 consumables (РМ) in Materials DB', async () => {
      if (U004_CONSUMABLES.length === 0) {
        logger.info('No U004 consumables to create (consumables list is empty).');
        return;
      }
      for (const c of U004_CONSUMABLES) {
        logger.info(`Creating consumable material in Materials DB: "${c.name}"`);
        await materialsPage.createConsumable(c.name, testInfo);
      }
    });
    await allure.step('Step 08: Create 1 detail (Д) for U004 spec', async () => {
      await partsDatabasePage.createOneDetail(U004_DETAILS[0].name);
    });
    await allure.step('Step 09: Create 3 CBEDs (СБ) for U004 spec', async () => {
      for (const a of U004_ASSEMBLIES) {
        await partsDatabasePage.createOneCBED(a.name, '-');
      }
    });
    await allure.step('Step 10: Create U004 product with full specification from config', async () => {
      await partsDatabasePage.createProductWithSpecification(
        U004_PRODUCT_NAME,
        {
          assemblies: U004_ASSEMBLIES,
          details: U004_DETAILS,
        ...
        { articleNumber: U004_PRODUCT_NAME, designation: '-' },
      );
    });
  });
  ...
  test('TestCase 01 - Редактирование изделия - добавление потомка (СБ) (Editing a product - adding a descendant (СБ))', async ({ browser, page }) => {
    ...
    await allure.step('Setup: Clean up U004 product specifications', async () => {
      console.log('Setup: Clean up U004 product specifications');
    ...
    // Page titles check vs JSON test data disabled (design may use h3/h4 or different structure)
    ...
    await allure.step(
      ...
      async () => {
        ...
        await expectSoftWithScreenshot(
          page,
          async () => {
            expect.soft(secondCellValue).toContain(U004_PRODUCT_NAME);
          },
    ...
    await allure.step(
      ...
      async () => {
        ...
        detailvalue_original_before_changequantity = await shortagePage.getQuantityByLineItem(tableData_original, U004_FIRST_DETAIL_NAME);
    ...
    await allure.step('Step 21: Проверяем, что в модальном окне есть две таблицы. (We check that there are two tables in the modal window.)', async () => {
      ...
      // Scope both tables to the open Specification-ModalBaseCbed dialog. Use the cbed wrapper so table2/search are always the right panel (Сборочная единица), not the left (Изделие).
      const modalLocator = page.locator(SelectorsPartsDataBase.EDIT_PAGE_ADD_СБ_RIGHT_DIALOG_OPEN);
      table1Locator = modalLocator.locator(SelectorsPartsDataBase.MAIN_PAGE_ИЗДЕЛИЕ_TABLE);
      const cbedSection = modalLocator.locator(SelectorsPartsDataBase.EDIT_PAGE_ADD_СБ_RIGHT_DIALOG_CBED_WRAPPER);
    ...
    if (!searchItemExists) {
      ...
      await allure.step(
        ...
        async () => {
          ...
          // Wait for loading
          await page.waitForLoadState('networkidle');
          await table2Locator!.locator(SelectorsPartsDataBase.TABLE_SEARCH_INPUT).fill(U004_FIRST_ASSEMBLY_NAME); //DATATESTID
          await page.waitForLoadState('networkidle');
          await page.waitForTimeout(1000);
      ...
      await allure.step(
        ...
        async () => {
          ...
          await expectSoftWithScreenshot(
            page,
            async () => {
              await expect.soft(table2Locator!.locator(SelectorsPartsDataBase.TABLE_SEARCH_INPUT)).toHaveValue(U004_FIRST_ASSEMBLY_NAME); //DATATESTID
            },
    ...
    await allure.step('Step 35: Нажимаем на кнопку "Сохранить". (Press the save button)', async () => {
      ...
      await shortagePage.clickMainSaveButtonAndDismissKitsDeactivationModalIfPresent();
    });
    //let tableData2: { groupName: string; items: string[][] }[] = [];
    await allure.step(
    ...
    await allure.step('Step 39: Очистка после теста. (Cleanup after test)', async () => {
      ...
      await allure.step('Step 39 sub step 3: find the bottom table', async () => {
        ...
        const selectedPartNumber = U004_FIRST_ASSEMBLY_NAME; // Replace with actual part number
        const modal = await page.locator(SelectorsPartsDataBase.EDIT_PAGE_ADD_СБ_RIGHT_DIALOG_DIALOG);
