...
const U004_FIRST_CONSUMABLE_QUANTITY = U004_CONSUMABLES[0].quantity;
...
export const runU004_6 = () => {
  ...
  test('TestCase 11 - Delete a Material Before Saving', async ({ page }, testInfo) => {
    ...
    await allure.step(
      'Step 09b: Check if ПД already in bottom table with required quantity',
      async () => {
        ...
        if (itemAlreadyInBottomTableПД) {
          logger.info('ПД already in bottom table. Skipping search and add.');
        }
      },
    );
    ...
    await allure.step('Step 15: Нажимаем по bottom кнопке "Добавить" в модальном окне (Click on the bottom "Добавить" button in the modal window)', async () => {
      ...
      await expectSoftWithScreenshot(
        ...
        async () => {
          ...
          const found = await shortagePage.isStringInNestedArray(nested, U004_FIRST_STANDARD_PART_NAME);
      ...
      await allure.step('Step 09c: ПД already in bottom table — close dialog', async () => {
        const cancelBtn = modal.locator(SelectorsPartsDataBase.EDIT_PAGE_ADD_ПД_RIGHT_DIALOG_CANCEL_BUTTON);
    ...
    await allure.step('Step 17: Подтвердите, что элемент теперь находится в массиве. (Confirm that the item is now in the array)', async () => {
      ...
      const result = await shortagePage.isStringInNestedArray(nestedArray, U004_FIRST_STANDARD_PART_NAME); // Output: true
    ...
    await allure.step('Step 18: Удалить элемент без сохранения. (Remove the item without saving)', async () => {
      ...
      await allure.step('Step 007 sub step 3: find the bottom table', async () => {
        const selectedPartNumber = U004_FIRST_STANDARD_PART_NAME;
  ...
  test('TestCase 12 - Удалить сохраненный материал (Remove Saved Material)', async ({ page }, testInfo) => {
    ...
    await allure.step(
      'Step 09b: Check if РМ already in bottom table with required quantity',
      async () => {
        await page.waitForLoadState('networkidle');
        itemAlreadyInBottomTableРМ = await shortagePage.checkItemExistsInBottomTable(
          ...
          U004_FIRST_CONSUMABLE_QUANTITY,
        );
        if (itemAlreadyInBottomTableРМ) {
          logger.info('РМ already in bottom table. Skipping search and add.');
        }
      },
    );
    ...
    if (!itemAlreadyInBottomTableРМ) {
      await allure.step('Step 10: Найдите элемент, который мы собираемся добавить.. (Sesarch for the item we are going to add)', async () => {
        await page.waitForLoadState('load');
        await table3Locator!.locator(SelectorsPartsDataBase.MODAL_BASE_MATERIAL_TABLE_LIST_TABLE_ITEM_SEARCH_INPUT_DROPDOWN_INPUT).fill(U004_FIRST_CONSUMABLE_NAME);
        await table3Locator!.locator(SelectorsPartsDataBase.MODAL_BASE_MATERIAL_TABLE_LIST_TABLE_ITEM_SEARCH_INPUT_DROPDOWN_INPUT).press('Enter');
        await page.waitForLoadState('load');
        // Optionally, validate that the search input is visible
        await expectSoftWithScreenshot(
      ...
      await allure.step('Step 11: Проверяем, что в найденной строке таблицы содержится значение переменной (We check that the found table row contains the value of the variable)', async () => {
        ...
        await expectSoftWithScreenshot(
          page,
          async () => {
            expect.soft(firstCellValue).toContain(U004_FIRST_CONSUMABLE_NAME);
          },
    ...
    await allure.step('Step 15: Нажимаем по bottom кнопке "Добавить" в модальном окне (Click on the bottom "Добавить" button in the modal window)', async () => {
      ...
      await expectSoftWithScreenshot(
        ...
        async () => {
          const specTable = await shortagePage.parseStructuredTable(page, SelectorsPartsDataBase.EDIT_PAGE_SPECIFICATIONS_TABLE);
          const nested = specTable.map(group => group.items).flat();
          const found = await shortagePage.isStringInNestedArray(nested, U004_FIRST_CONSUMABLE_NAME);
          expect.soft(found).toBeTruthy();
        },
      ...
      await allure.step('Step 09c: Close РМ dialog (item already in bottom table)', async () => {
        const cancelBtn = modal.locator(SelectorsPartsDataBase.EDIT_PAGE_ADD_РМ_RIGHT_DIALOG_CANCEL_BUTTON);
        await shortagePage.waitAndHighlight(cancelBtn);
        await cancelBtn.click();
        await page.waitForTimeout(TIMEOUTS.MEDIUM);
        await modal.waitFor({ state: 'hidden', timeout: WAIT_TIMEOUTS.STANDARD }).catch(() => {});
      });
      firstCellValue = U004_FIRST_CONSUMABLE_NAME;
    }
    ...
    await allure.step('Step 17: refresh and confirm saved. (refresh and confirm saved)', async () => {
      ...
      await page.waitForLoadState('load');
      const nestedArray = tableData_full.map(group => group.items).flat();
      const result = await shortagePage.isStringInNestedArray(nestedArray, U004_FIRST_CONSUMABLE_NAME); // Output: true
      await expectSoftWithScreenshot(
        page,
        async () => {
          expect.soft(result).toBeTruthy();
