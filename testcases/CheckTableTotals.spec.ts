...
export const runCheckTableTotals = (isSingleTest: boolean, iterations: number) => {
  ...
  test('Case 01 - Check Металлообработка Table Totals', async ({ page }) => {
        ...
        if (bracketMatch) {
          ...
          await allure.step(`Step 4: Validate table totals for "${cardTitle}"`, async () => {
            ...
            do {
              ...
              await Promise.race([page.waitForLoadState('networkidle'), page.waitForTimeout(TIMEOUTS.EXTENDED)]);
  ...
  test('Case 02 - Check Сборка Table Totals', async ({ page }) => {
    test.setTimeout(TEST_TIMEOUTS.EXTENDED);
    const pageObj = new PageObject(page);
        ...
        if (bracketMatch) {
          ...
          await allure.step(`Step 3: Validate table totals for "${cardTitle}"`, async () => {
            ...
            do {
              ...
              const tableSlotResolved = await tableScrollSlot.waitFor({ state: 'attached', timeout: WAIT_TIMEOUTS.SHORT }).catch(() => null);
          ...
          const item0Selected = await switchItem0.evaluate((el: HTMLElement) => {
            return el.getAttribute('aria-pressed') === 'true' || el.classList.contains('selected') || el.classList.contains('active');
          });
