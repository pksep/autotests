/**
 * @file PartsDatabaseTableHelper.ts
 * @purpose Helper for Parts Database table/specification processing logic (extracted from PartsDatabasePage).
 */

import { Page, Locator, expect } from '@playwright/test';
import type { Item, GlobalTableData } from './PartsDatabaseTypes';
import { TableHelper } from './TableHelper';
import { ElementHelper } from './ElementHelper';
import logger from '../utils/logger';
import testData from '../../testdata/PU18-Names.json';
import { allure } from 'allure-playwright';

export class PartsDatabaseTableHelper {
  ...
  async processTableData(
    ...
    const addToAll = (item: Item) => {
      ...
      const globalExistingItem = globalTableData.ALL.get(uniqueKey);
      if (globalExistingItem) {
        globalExistingItem.quantity += item.quantity;
      } else {
        globalTableData.ALL.set(uniqueKey, item);
      }
      logger.info(`Global ALL group updated: Current size = ${globalTableData.ALL.size}`);
    };
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (isDataRow && currentGroup) {
        ...
        if (id && name && quantity) {
          ...
          if (Array.isArray(globalTableData[currentGroup as keyof GlobalTableData])) {
            if (currentGroup === 'ПД') {
              let isDuplicate = false;
              for (const row of globalTableData[currentGroup]) {
              ...
              if (!isDuplicate) {
                (globalTableData[currentGroup] as Item[]).push(item);
                logger.info(`Added new item to ПД group: "${item.material}"`);
              }
            } else {
              (globalTableData[currentGroup as keyof GlobalTableData] as Item[]).push(item);
            }
          }
  ...
  async processTableDataAndHandleModals(
    ...
    const groups = await this.processTableData(table, title, parentQuantity, globalTableData);
    await this.processGroupRows(groups.Д, 'Д', page, parentQuantity, globalTableData);
    await this.processGroupRows(groups.ПД, 'ПД', page, parentQuantity, globalTableData);
    await this.processGroupRows(groups.РМ, 'РМ', page, parentQuantity, globalTableData);
    await this.processSBGroupRows(groups.СБ, page, parentQuantity, globalTableData);
    return groups;
  }
      ...
      switch (groupType) {
        case 'Д': {
          ...
          if (elementValue !== item.name) {
            ...
            if (!globalTableData.Д.some(existingItem => existingItem.name === item.name)) {
              globalTableData.Д.push(item);
            }
          }
          ...
          if (!elementValue) {
            ...
            tableZagContent = tableZagContent.replace(/<!--v-if-->/g, '').trim();
            const hasTableRows = /<tr[^>]*>/i.test(tableZagContent);
            if (blankMassValue?.trim() === '0' && !hasTableRows) {
              const existingItem = globalTableData.ПМ.find(existingItem => {
        ...
        case 'ПД': {
          ...
          let el2 = await modal2.locator('[data-testid="ModalMaterialInformation-Title"]').last();
          ...
          el2 = await modal2.locator('[data-testid="ModalMaterialInformation-NameValue"]').last();
          await el2.waitFor({ state: 'attached', timeout: 30000 });
          await el2.evaluate((element: HTMLElement) => {
            element.style.border = '3px solid red';
            element.style.backgroundColor = 'yellow';
        ...
        case 'РМ': {
          ...
          let el3 = await modal3.locator('[data-testid="ModalMaterialInformation-Title"]').last();
          ...
          el3 = await modal3.locator('[data-testid="ModalMaterialInformation-NameValue"]').last();
          await el3.waitFor({ state: 'attached', timeout: 30000 });
          await el3.evaluate((element: HTMLElement) => {
            element.style.border = '3px solid red';
            element.style.backgroundColor = 'yellow';
  ...
  async processSBGroupRows(
    ...
    for (const item of rows) {
      ...
      const titleLoc = page.locator('[data-testid="ModalCbed-Title"]').last();
      await titleLoc.waitFor({ state: 'attached', timeout: 30000 });
      await titleLoc.evaluate((element: HTMLElement) => {
        element.style.border = '3px solid red';
        element.style.backgroundColor = 'yellow';
      });
      const eleText = await titleLoc.textContent();
      if (eleText?.trim() !== testData.titles.СБ.label) {
        logger.error('Incorrect modal title for Type СБ');
        expect(eleText?.trim()).toBe(testData.titles.СБ.label);
      }
      await page.waitForTimeout(1000);
      const nameLoc = page.locator('[data-testid="ModalCbed-Text-Name"]').last();
      await nameLoc.waitFor({ state: 'attached', timeout: 30000 });
      await nameLoc.evaluate((element: HTMLElement) => {
        element.style.border = '3px solid red';
        element.style.backgroundColor = 'yellow';
      });
      const elemText = await nameLoc.textContent();
      if (elemText !== item.name) {
        logger.error('Incorrect Product Name for Type СБ');
        expect(elemText?.trim()).toBe(item.name);
      }
      await page.waitForTimeout(1000);
      const designationLoc = page.locator('[data-testid="ModalCbed-Text-Designation"]').last();
      await designationLoc.waitFor({ state: 'attached', timeout: 30000 });
      await designationLoc.evaluate((element: HTMLElement) => {
        element.style.border = '3px solid red';
        element.style.backgroundColor = 'yellow';
      });
      const elemeText = await designationLoc.textContent();
      if (elemeText !== item.partNumber) {
        logger.error('Incorrect Part Number for Type СБ');
        expect(elemeText?.trim()).toBe(item.partNumber);
      }
      const eRow = page.locator(`[data-testid="${item.dataTestId}"]`).last();
      ...
      item.quantity *= parentQuantity;
      const subGroups = await this.processTableDataAndHandleModals(tableInModal, page, title ?? '', item.quantity, globalTableData);
      logger.info('Processed Sub-Groups for СБ item:', subGroups);
      await page.mouse.click(1, 1);
    }
  }

  async processProduct(row: Locator, page: Page, title: string, globalTableData: GlobalTableData): Promise<void> {
    ...
    const groups = await this.processTableDataAndHandleModals(table, page, title, parentQuantity, globalTableData);
    logger.info('Processed Groups:');
    logger.info(groups);
  }

  async getProductSpecificationsTable(row: Locator, page: Page, title: string, globalTableData: GlobalTableData): Promise<void> {
    ...
    await this.elementHelper.findAndClickElement(page, 'BaseDetals-Button-EditProduct', 500); // Clicked the edit button
    const pageLocator = page.locator(`[data-testid="App-RouterView"]`);
    await pageLocator.waitFor({ state: 'attached', timeout: 30000 });
    await this.elementHelper.findAndClickElement(page, 'TableSpecification-Button-FullSpecification', 500); // Clicked the full specification button
    ...
    const checkForDuplicates = async (tableId: string, tableName: string, partNumberId: string | null, nameId: string): Promise<void> => {
       ...
       */
      const designationColumnIndex = partNumberId ? await this.tableHelper.findColumn(page, tableId, partNumberId) : -1; // Use -1 if partNumberId is not provided
      const nameColumnIndex = await this.tableHelper.findColumn(page, tableId, nameId);
     ...
     * @param {keyof GlobalTableData} globalKey - The global dataset key to validate against.
     * @returns {Promise<void>} - A promise that resolves when the comparison is complete.
     */
    const compareTotals = async (modalLocator: Locator, testId: string, globalKey: keyof GlobalTableData): Promise<void> => {
      ...
      const globalValue =
        globalKey === 'ALL'
          ? globalTableData.ALL.size // Global value for "ALL" is the size of the Map
          : globalTableData[globalKey].length; // Global value for other keys is array length
    ...
    const compareItemsCB = async (modalLocator: Locator, tableId: string): Promise<void> => {
       ...
       */
      const partNoIndex = await this.tableHelper.findColumn(page, tableId, 'ModalComplect-CbedsTableHead-Designation');
      const partNameIndex = await this.tableHelper.findColumn(page, tableId, 'ModalComplect-CbedsTableHead-Name');
      const partCountIndex = await this.tableHelper.findColumn(page, tableId, 'ModalComplect-CbedsTableHead-Count');
       ...
       */
      const arrayData = globalTableData[globalKey];
    ...
    const compareItemsD = async (modalLocator: Locator, tableId: string): Promise<void> => {
       ...
       * These indices are used to locate specific cells within each row.
       */
      const parentPartIndex = await this.tableHelper.findColumn(page, tableId, 'ModalComplect-DetalsTableHead-CbedDesignation');
      const partNoIndex = await this.tableHelper.findColumn(page, tableId, 'ModalComplect-DetalsTableHead-Designation');
      const partNameIndex = await this.tableHelper.findColumn(page, tableId, 'ModalComplect-DetalsTableHead-Name');
      const partMaterialIndex = await this.tableHelper.findColumn(page, tableId, 'ModalComplect-DetalsTableHead-Zag');
      const partCountIndex = await this.tableHelper.findColumn(page, tableId, 'ModalComplect-DetalsTableHead-Count');
    ...
    const compareItemsCon = async (modalLocator: Locator, tableId: string): Promise<void> => {
       ...
       * These indices are used to locate specific cells within each row.
       */
      const nameIndex = await this.tableHelper.findColumn(page, tableId, 'ModalComplect-ConsumableMaterialsTableHead-Name');
      const quantityIndex = await this.tableHelper.findColumn(page, tableId, 'ModalComplect-ConsumableMaterialsTableHead-Count');
    ...
    const validateMaterialExistence = async (modalLocator: Locator, detailsTableId: string, materialsTableId: string): Promise<void> => {
      try {
        ...
        const detailsMaterialNameIndex = await this.tableHelper.findColumn(page, detailsTableId, 'ModalComplect-DetalsTableHead-Zag');
        ...
        const materialsTableNameIndex = await this.tableHelper.findColumn(page, materialsTableId, 'ModalComplect-DetailMaterialsTableHead-Name');
