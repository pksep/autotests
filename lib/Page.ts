...
export class PageObject extends AbstractPage {
  ...
  async checkTableRowOrdering(
    ...
    const rowData = await Promise.all(
      filteredRows.map(async (row, rowIndex) => {
        const cells = await row.$$('td');
        //logger.info(`Row ${rowIndex} has ${cells.length} cells.`); // Debugging log

        // Log all column content for the current row
        for (let colIndex = 0; colIndex < cells.length; colIndex++) {
          try {
            const cellContent = await cells[colIndex].innerText();
            //logger.info(
            //  `Row ${rowIndex}, Column ${colIndex}: "${cellContent}"`
            //);
          } catch (error) {
            logger.error(
              `Error getting innerText for Row ${rowIndex}, Column ${colIndex}:`,
              error
            );
          }
        }
