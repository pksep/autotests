...
export class CreateCompleteSetsPage extends PageObject {
  ...
  async disassemblyModalWindow(name: string, designation: string) {
    const modalWindow = this.page.locator('dialog[data-testid="ComplectKit-Modal-UncomplectKit"]');
   ...
   */
  async checkDisassemblyQuantity(locator: string, qunatity: string, qunatityOrder?: string) {
    ...
    if (qunatityOrder) {
      ...
      expect(await modalWindowLaunchIntoProduction.locator('input').inputValue()).toBe(qunatityOrder);
    }
    if (!qunatityOrder) {
      expect(await modalWindowLaunchIntoProduction.locator('input').inputValue()).toBe(qunatity);
    }
  }
}
