...
export class CreateStockPage extends PageObject {
  ...
  async checkingTheQuantityInStock(serachName: string, tableSelection: TableSelection) {
    ...
    if (tableSelection === TableSelection.product) {
      remainingStock = await this.getValueOrClickFromFirstRow(tableLocator, 3);
    } else {
      remainingStock = await this.getValueOrClickFromFirstRow(tableLocator, 2);
    }
    // Output to the console
    logger.log(`Количество ${serachName} на складе ${remainingStock}`);

    return remainingStock;
  }

  async checkingTheQuantityInStockNew(serachName: string, tableSelection: TableSelection) {
    ...
    const numberColumn = await this.findColumn(this.page, '.scroll-table.detal', 'OstatkiPCBDTable-Header-Stock');
    logger.log('numberColumn: ', numberColumn);
    // Upd:
    const remainingStock = await this.getValueOrClickFromFirstRow(tableLocator, 2);

    // Output to the console
    logger.log(`Количество ${serachName} на складе до оприходования  ${remainingStock}`);
