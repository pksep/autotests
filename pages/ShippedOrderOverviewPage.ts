...
export class CreateShippedOrderOverviewPage extends PageObject {
   ...
   */
  async getValueOrClickFromFirstRowNoTheadBUG(locator: string, cellIndex: number, click: Click = Click.No, dblclick: Click = Click.No) {
  ...
  async shippingInformationModalWindow() {
    ...
    await expect(modalWindow.locator('h3', { hasText: ' Информация об Отгрузке ' })).toBeVisible();
    await expect(modalWindow.locator('h3', { hasText: 'Примечание' })).toBeVisible();
    await expect(modalWindow.locator('h3', { hasText: 'Файлы' })).toBeVisible();
    await expect(modalWindow.locator('h3', { hasText: ' Информация об отгруженном товаре ' })).toBeVisible();

    // Check button
    await this.clickButton('  Отменить  ', '[data-testid="ModalShComlitUpdate-ButtonControlCancel"]', Click.No);
    await this.clickButton(' Отменить отгрузку ', '[data-testid="ModalShComlitUpdate-ButtonControlCancelShipment"]', Click.No);
    await this.clickButton(' Внести изменения в отгрузку ', '[data-testid="ModalShComlitUpdate-ButtonControlUpdateShipment"]', Click.No);
  }
}
