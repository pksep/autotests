...
export class AnalyticsAPI extends APIPageObject {
    ...
    async createKPI(request: APIRequestContext, kpiData: any, userId: string) {
        logger.info(`Creating KPI:`, kpiData);

        const response = await request.post(ENV.API_BASE_URL + 'api/analytics/kpis/create', {
            headers: {
                'Content-Type': 'application/json',
                'user-id': userId
            },
        ...
        if (response.ok()) {
            logger.info(`KPI created successfully`);
            return { status: response.status(), data: responseData };
        } else {
            logger.error(`Failed to create KPI, status: ${response.status()}`);
            return { status: response.status(), data: responseData };
        }
    }
