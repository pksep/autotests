...
export class LogsAPI extends APIPageObject {
    ...
    async createLogEntry(request: APIRequestContext, logData: any, userId: string) {
        logger.info(`Creating log entry:`, logData);

        const response = await request.post(ENV.API_BASE_URL + 'api/logs', {
            headers: {
        ...
        if (response.ok()) {
            ...
            logger.info(`Successfully created log entry`);
            return { status: response.status(), data: responseData };
        } else {
            logger.error(`Failed to create log entry, status: ${response.status()}`);
