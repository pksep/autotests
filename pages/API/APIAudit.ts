...
export class AuditAPI extends APIPageObject {
    ...
    async createAuditEntry(request: APIRequestContext, auditData: any, userId: string) {
        logger.info(`Creating audit entry:`, auditData);

        const response = await request.post(ENV.API_BASE_URL + 'api/audit/logs/create', {
            headers: {
        ...
        if (response.ok()) {
            ...
            logger.error(`Failed to create audit entry, status: ${response.status()}`);
            return { status: response.status(), data: responseData };
        }
    }
