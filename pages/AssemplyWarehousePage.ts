...
export class CreateAssemblyWarehousePage extends PageObject {
  ...
  /**
   * Archives all assembly warehouse tasks for the given item name (repeatedly: select first row, archive, confirm, search again until no rows).
   * Call when already on Assembly Warehouse page. Used by U002 Case 15 (CBED) and Case 18 (IZD).
   * @returns Number of tasks archived.
   */
  async archiveAllAssemblyTasksForItem(
    itemName: string,
    ...
    let rows = this.page.locator(`${SelectorsAssemblyWarehouse.ZAKAZ_SCLAD_TABLE_ASSEMBLY_WARHOUSE} tbody tr`);
    let rowCount = await rows.count();
    logger.info(`Found ${rowCount} task(s) to archive for ${itemName}`);

    while (rowCount > 0 && archivedCount < maxArchives) {
      const firstRow = rows.first();
      await firstRow.waitFor({ state: 'visible', timeout: WAIT_TIMEOUTS.SHORT });
      const checkbox = firstRow.locator(SelectorsAssemblyWarehouse.DATA_CELL).first();
      ...
      rows = this.page.locator(`${SelectorsAssemblyWarehouse.ZAKAZ_SCLAD_TABLE_ASSEMBLY_WARHOUSE} tbody tr`);
      ...
      logger.info(`Archived 1 task, ${rowCount} task(s) remaining for ${itemName}`);
    }

    if (archivedCount >= maxArchives) {
      logger.info(`Reached maximum archive limit (${maxArchives}), stopping`);
    }
    logger.info(`Completed archiving ${archivedCount} task(s) for ${itemName}`);
    ...
    await allure.step(`Archive orders for items with prefixes: ${itemPrefixes.join(', ')}`, async () => {
      ...
      for (const itemPrefix of itemPrefixes) {
        try {
          ...
          const tableExists = await this.page
            .locator(SelectorsAssemblyWarehouse.ZAKAZ_SCLAD_TABLE_ASSEMBLY_WARHOUSE)
            .isVisible()
            .catch(() => false);
          if (!tableExists) {
          ...
          await this.searchAndWaitForTable(itemPrefix, SelectorsAssemblyWarehouse.ZAKAZ_SCLAD_TABLE_ASSEMBLY_WARHOUSE, SelectorsAssemblyWarehouse.ZAKAZ_SCLAD_TABLE_ASSEMBLY_WARHOUSE, {
            searchInputDataTestId: SelectorsAssemblyWarehouse.ZAKAZ_SCLAD_TABLE_ASSEMBLY_SEARCH_INPUT,
          });
          ...
          for (let i = 0; i < rowCount; i++) {
            ...
            try {
              ...
              const ordersListModal = this.page.locator(`${SelectorsOrderedFromSuppliers.MODAL_SHIPMENTS_TO_IZED_RIGHT_MENU_MODAL}[open]`);
              await this.highlightElement(ordersListModal, { border: '3px solid red' });

              const ordersListModalTable = this.page.locator(SelectorsOrderedFromSuppliers.MODAL_SHIPMENTS_TO_IZED_TABLE_SCLAD);
