...
export class CreateOrderedFromSuppliersPage extends PageObject {
  ...
  async selectSupplier(supplier: Supplier) {
    ...
    const nextReady = this.page.locator(`${SelectorsOrderedFromSuppliers.TABLE_MODAL_WINDOW}, ${SelectorsOrderedFromSuppliers.MODAL_ADD_ORDER_PRODUCTION_TABLE_PAGE}`).first();
  ...
  async compareOrderNumbers(orderNumber: string) {
    ...
    await mainTable.evaluate((el: unknown) => {
      (el as { style: Record<string, string> }).style.border = '2px solid red';
    });
    ...
    await orderRow.evaluate((el: unknown) => {
  ...
  //         async () => {
  //             await this.tableHelper.findTable(selector);
  //             await this.page.waitForLoadState("networkidle");
  //         }
  //     );
  ...
  //         await this.tableHelper.waitingTableBody(`[data-testid="${CONST.TABLEMODALWINDOW}"]`);
  ...
  //         await this.page.waitForTimeout(1000);
  //         const successNotif = await this.notification.extractNotificationMessage(this.page);
  //         if (successNotif) {
  ...
  //         await this.tableHelper.searchTable(
  //             name,
  //             `[data-testid="${CONST.ORDER_SUPPLIERS_TABLE_ORDER_TABLE}"]`
  //         );

  //         await this.tableHelper.waitingTableBody(
  //             `[data-testid="${CONST.ORDER_SUPPLIERS_TABLE_ORDER_TABLE}"]`
  //         );
  //     });
  ...
  // }
  async launchIntoProductionSupplier(name: string, quantityOrder: string, supplier: Supplier): Promise<{ quantityLaunchInProduct: string; checkOrderNumber: string }> {
    // Quantity launched into production
    ...
    await allure.step('Step 3: Click on the Launch on Production button', async () => {
    ...
    await allure.step('Step 5: Search product', async () => {
      ...
      await this.tableHelper.waitingTableBody(test);
      await this.tableHelper.searchTable(name, test, SelectorsOrderedFromSuppliers.MODAL_ADD_ORDER_PRODUCTION_TABLE_SEARCH_INPUT_DATA_TESTID);
      await this.page.waitForLoadState('networkidle');
      await this.waitForTimeout(500);
      await this.tableHelper.waitingTableBody(test);
    });

    await allure.step('Step 6: Click the checkbox in the first row', async () => {
    ...
    await allure.step('Step 7: We find the value in the cell ordered for production and get the value', async () => {
    ...
    await allure.step('Step 7.5: Click Choose button to add item to bottom table', async () => {
    ...
    await allure.step('Step 8: Enter the quantity into the input cell', async () => {
      // Scroll down to see the bottom table (runs in browser context)
   ...
   */
  async launchIntoProductionSupplierByPrefix(name: string, quantityOrder: string, supplier: Supplier): Promise<{ quantityLaunchInProduct: string; checkOrderNumber: string }> {
    // Quantity launched into production
    let quantityLaunchInProduct: string = '0';
    let checkOrderNumber: string = '';
    ...
    await allure.step('Step 2: Open the shortage assemblies page', async () => {
      await this.tableHelper.findTable(SelectorsOrderedFromSuppliers.ORDERED_SUPPLIERS_PAGE_TABLE);
      await this.page.waitForLoadState('networkidle');
    });
    ...
    await allure.step('Step 5: Search product by prefix', async () => {
      await this.waitForTimeout(500);
      await this.tableHelper.waitingTableBody(SelectorsOrderedFromSuppliers.TABLE_MODAL_ADD_ORDER_PRODUCTION_TABLE);
      ...
      await this.tableHelper.searchTable(searchPrefix, SelectorsOrderedFromSuppliers.TABLE_MODAL_ADD_ORDER_PRODUCTION_TABLE, SelectorsOrderedFromSuppliers.MODAL_ADD_ORDER_PRODUCTION_TABLE_SEARCH_INPUT_DATA_TESTID);
      await this.page.waitForLoadState('networkidle');
      await this.waitForTimeout(500);
      await this.tableHelper.waitingTableBody(SelectorsOrderedFromSuppliers.TABLE_MODAL_ADD_ORDER_PRODUCTION_TABLE);
    });

    await allure.step('Step 6: Click checkboxes for all matching rows', async () => {
    ...
    await allure.step('Step 8: Enter the quantity into all input cells', async () => {
      // Scroll down to see the bottom table (runs in browser context)
      await this.page.evaluate(() => {
        // eslint-disable-next-line no-undef
        window.scrollTo(0, document.body.scrollHeight);
